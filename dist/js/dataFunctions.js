...
export const getCoordsFromApi = async (entryText, units) => {
    ...
    // const url = `https://api.openweathermap.org/data/2.5/weather?${flag}=${entryText}&units=${units}&appid=${WEATHERKEY}`;

    // const encodedUrl = encodeURI(url);
    // try {
    //     const dataStreams = await fetch(encodedUrl)
    //     const jsonData = dataStreams.json()
...
export const cleanText = (entry) => {
    const regex = /{2,w}/g;
    const entryText = entry.replaceAll(regex, " ").trim();
    return entryText;
}
