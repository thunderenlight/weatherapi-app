...
export const displayApiError = (code) => {
    const properMsg = toProperCase(code.message);
...
const updateWeatherLocationHeader = (msg) => {
    const h1 = document.getElementById("currentConditions__location");
...
export const updateDisplay = (weatherJson, locationObj) => {
  ...
  const setBGImage = (weatherClass) => {
    ...
    document.documentElement.classList.forEach((img) => {
      ...
      console.log("here BG", img === weatherClass)
    });
  };
  ...
  const displayCurrentConditions = (currentConditionsArray) => {
    const ccContainer = document.getElementById("current__conditions");
